...
class MPNNEmbeddings:
    def __init__(self, dataset_path, features_dir=None, features_generator=None,
        ...
        Args:
            ...
            - features_generator (str): features generators.
    ...
    def load_sdf_files(self, input_file, clean_mols=True):
        ...
        try:
            ...
            raw_df['mol_id'] = raw_df['gdb9_index']
            raw_df = raw_df.set_index('mol_id')
        except KeyError:
            raw_df = pd.read_csv(str(input_file) + '.csv')
            raw_df['mol_id'] = raw_df['mol_id'].apply(lambda x: int(x.split('_')[-1]))
            raw_df = raw_df.set_index('mol_id')
        return pd.concat([mol_df, raw_df], axis=1, join='inner').reset_index()
    ...
    def process(self):
        ...
        if self.use_data_saving and os.path.exists(output_path):
            ...
            _, x_all, y_all = pickle.load(open(output_path, 'rb'))
            self.configs.dim_features = len(x_all[0][0]) if x_all[0][0] is not None else 0
            ...
            pickle.dump((smiles_all, x_all, y_all), open(output_path, "wb"))
    ...
    def load_data_from_smiles(self, x_smiles, labels):
        ...
        for smiles, label in zip(x_smiles, labels):
            try:
                ...
                if self.configs.atom_descriptors == 'feature':
                    atom_features = self.get_atom_features(smiles)
                elif self.configs.atom_descriptors == 'descriptor':
